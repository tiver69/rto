...
public interface StopRepository extends CrudRepository<Stop, StopKey> {
    ...
    @Query("SELECT new com.railway.ticketoffice.dto.request.train.TrainInfoDto(" +
            ...
            "(SELECT s.station.name FROM Stop s WHERE s.train.id = departure.train.id AND s.stopOrder = 0), " +
            "(SELECT s1.station.name FROM Stop s1 WHERE s1.train.id = departure.train.id AND s1.stopOrder = (SELECT MAX(s2.stopOrder) FROM Stop s2 WHERE s2.train.id = departure.train.id)), " +
            ...
            "AND departure.stopOrder < destination.stopOrder " +
    ...
    @Query("SELECT s FROM Stop s WHERE s.train.id = :trainId " +
            "AND s.stopOrder >= (SELECT s1.stopOrder FROM Stop s1  WHERE s1.station.id = :departureStationId AND s1.train.id = :trainId) " +
            "AND s.stopOrder <= (SELECT s2.stopOrder FROM Stop s2  WHERE s2.station.id = :destinationStationId AND s2.train.id = :trainId) " +
    ...
    @Query("SELECT SUM(s.price) + " +
            ...
            "FROM Stop s " +
            "WHERE s.stopOrder > (SELECT s1.stopOrder FROM Stop s1 WHERE s1.station.id = :departureStationId AND s1.train.id = :trainId) " +
            "AND " +
            "s.stopOrder <= (SELECT s2.stopOrder FROM Stop s2 WHERE s2.station.id = :destinationStationId AND s2.train.id = :trainId) " +
            "AND " +
            "s.train.id = :trainId")
